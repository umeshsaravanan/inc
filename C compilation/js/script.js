...
async function initWebSocket() {
    ...
    socket.onmessage = function (event) {
        ...
        if (event.data.indexOf('running') >= 0) {
            document.getElementById("inner-text-area").placeholder = "Waiting for input...";
            return;
        }
...
document.getElementById("inner-text-area").addEventListener("keydown",async function (event) {
    ...
    if (event.key === 'Enter') {
        ...
        printfStmt = '';
        await sendToServlet(lastLine.trim());
        resultArea.value += '\n';
        handleEnter(event);
    }
});
...
function customizeVariables(input) {
    ...
    const objectArray = customVar.split(/[;]+/).map(pair => {
        if (pair.indexOf('=') >= 0) {
            if (pair.length >= 1)
                return {
                    stmt: pair.trim().split(/[=]+/).map(word => word.trim()),
                    modified: false
                };
        } else {
            return {
    ...
    for (const obj of objectArray) {
        if (obj) {
            if (typeof(obj.stmt) === 'object' && obj.stmt[0].includes('[')) {
                        ...
                        if (stmt.includes('scanf')) {
                            ...
                            while ((index = stmt.indexOf('&', index)) >= 0) {
                                ...
                                if (variable.indexOf('[') >= 0) {
                                    appendToInputString(objectArray);
                                }
                                else {
...
function appendToInputString(objArray) {
    let flag = true; 

    for (let index = 0; index < objArray.length; index++) {
        if (typeof(objArray[index].stmt) === 'string' && objArray[index].stmt.trim() !== '' && objArray[index].modified === false) {
            inputString += (objArray[index].stmt + '\n');
            objArray[index].modified = true;
            flag = false;
        }
    }
...
//adding quotes to the user's custom input
...
//check for user's input variable from program's scanner line
...
//function to help non editable output container
...
async function customCompile() {
    ...
    document.getElementById('cr').style.display = 'none';
    const customInputs = document.getElementById('customSection').value.trim().split('\n');
    let output = 'Case 1:\n';
